...
import (
	...
	"sync"		//added this line for fixing the bug of gumball inventory count
)
...
func gumballProcessOrdersHandler(formatter *render.Render) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		var mutex = &sync.Mutex{} //added this line for fixing the bug of gumball inventory count
		...
		for i := 0; i < len(order_ids); i++ {
			...
			orders[order_id] = ord
			mutex.Lock()  //added this line for fixing the bug of gumball inventory count 
			count -= 1
			mutex.Unlock()    //added this line for fixing the bug of gumball inventory count
		}
